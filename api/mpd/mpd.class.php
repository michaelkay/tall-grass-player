...
define("MPD_CMD_PLAYLISTS",   "listplaylists");
define("MPD_CMD_PLAYLISTINFO","listplaylistinfo");
...
// playlist tag ID commands
define("MPD_CMD_ADD_TAG_ID",	"addtagid");
define("MPD_CMD_DEL_TAG_ID",	"cleartagid");

$mpd_debug = 1;

function addLog($text, $level = 0){
	global $mpd_debug;
    
    // Add some simple coloring to assist with debugging
    if($level == 0)	$style="background-color:lightgrey;border:thin solid grey;margin:5px;padding:5px";
    else if($level == 1) $style="background-color:white;border:thin solid grey;margin:5px;padding:5px";
    else if($level == 2) $style="background-color:lightgreen;border:thin solid grey;margin:5px;padding:5px";
    else $style="background-color:lightblue;border:thin solid grey;margin:5px;padding:5px";
    
	if ($mpd_debug) echo '<div style="'.$style.'">log:>'.htmlspecialchars($text).'</div>';
}
function addErr($err){
	global $mpd_debug;
	if ($mpd_debug) echo 'error:>'.htmlspecialchars($err).'<br>';
}
...
class mpd {
	...
	function Connect() {
		...
		try {
			...
			addErr('fsockopen() Error: $e->getMessage()');
			return NULL;
		}
	...
	function SendCommand($cmdStr,$arg1 = "",$arg2 = "",$arg3 = "") {
		addLog("mpd->SendCommand() / cmd: \"$cmdStr\", args: \"$arg1\" \"$arg2\" \"$arg3\"" );
	...
	function QueueCommand($cmdStr,$arg1 = "",$arg2 = "") {
		if ( $this->debugging ) addLog("mpd->QueueCommand() / cmd: ".$cmdStr.", args: ".$arg1." ".$arg2."\n");
		if ( ! $this->connected ) {
			...
			if ( $this->debugging ) addLog("mpd->QueueCommand() / return\n");
		}
		return TRUE;
	}
	...
	function SendCommandQueue() {
		if ( $this->debugging ) addLog("mpd->SendCommandQueue()");
		if ( ! $this->connected ) {
			...
			if ( is_null($respStr = $this->SendCommand($this->command_queue)) ) {
				...
				if ( $this->debugging ) addLog("mpd->SendCommandQueue() / response: '".$respStr."'");
			}
		}
		return $respStr;
	}
	...
	function AdjustVolume($modifier) {
		if ( $this->debugging ) addLog("mpd->AdjustVolume()");
		...
		if ( $this->debugging ) addLog("mpd->AdjustVolume() / return");
		return $ret;
	}
	...
	function SetVolume($newVol) {
		if ( $this->debugging ) addLog("mpd->SetVolume()");
		...
		if ( $this->debugging ) addLog("mpd->SetVolume() / return");
		return $ret;
	}
...
*/		
	...
	function GetDirTest($dir = "") {
		if ( $this->debugging ) addLog("mpd->GetDir()");
		$resp = $this->SendCommand(MPD_CMD_LSDIR,$dir);
		...
		if ( $this->debugging ) addLog("mpd->GetDir() / return ".print_r($dirlist)."");
		return $dirlist;
	}
	...
	function PLAddBulk($trackArray) {
		if ( $this->debugging ) addLog("mpd->PLAddBulk()");
		...
		$this->RefreshInfo();
		if ( $this->debugging ) addLog("mpd->PLAddBulk() / return");
		return $resp;
	}
	...
	function PLAdd($fileName) {
		if ( $this->debugging ) addLog("mpd->PLAdd()");
		if ( ! is_null($resp = $this->SendCommand(MPD_CMD_PLADD,$fileName))) $this->RefreshInfo();
		if ( $this->debugging ) addLog("mpd->PLAdd() / return");
		return $resp;
	}
	...
	function PLMoveTrack($origPos, $newPos) {
		if ( $this->debugging ) addLog("mpd->PLMoveTrack()");
		...
		if ( $this->debugging ) addLog("mpd->PLMoveTrack() / return");
		return $resp;
	}
	 ...
	 */
	function PLShuffle() {
		if ( $this->debugging ) addLog("mpd->PLShuffle()");
		if ( ! is_null($resp = $this->SendCommand(MPD_CMD_PLSHUFFLE))) $this->RefreshInfo();
		if ( $this->debugging ) addLog("mpd->PLShuffle() / return");
		return $resp;
	}
	...
	function PLLoad($file) {
		if ( $this->debugging ) addLog("mpd->PLLoad()");
		if ( ! is_null($resp = $this->SendCommand(MPD_CMD_PLLOAD,$file))) $this->RefreshInfo();
		if ( $this->debugging ) addLog("mpd->PLLoad() / return");
		return $resp;
	}
	...
	function PLSave($file) {
		if ( $this->debugging ) addLog("mpd->PLSave()");
		$resp = $this->SendCommand(MPD_CMD_PLSAVE,$file);
		if ( $this->debugging ) addLog("mpd->PLSave() / return");
		return $resp;
	}
	 ...
	 */
	function PLClear() {
		if ( $this->debugging ) addLog("mpd->PLClear()");
		if ( ! is_null($resp = $this->SendCommand(MPD_CMD_PLCLEAR))) $this->RefreshInfo();
		if ( $this->debugging ) addLog("mpd->PLClear() / return");
		return $resp;
	}

	/* PLRemove() 
	 * 
	 * Removes track <pos> from the playlist.
	 */
	function PLRemove($pos) {
		if ( $this->debugging ) addLog("mpd->PLRemove()");
		if ( ! is_numeric($pos) ) {
			$this->errStr = "PLRemove() : argument 1 must be a numeric value";
			return NULL;
		}
		if ( ! is_null($resp = $this->SendCommand(MPD_CMD_PLREMOVE,$pos))) $this->RefreshInfo();
		if ( $this->debugging ) addLog("mpd->PLRemove() / return");
		return $resp;
	}
	...
	/* PLAddTag() 
	 * 
	 * Adds a tag value for a song in the play queue. Used to add title/song information when files
	 * do not have the info in the metadata
	 */
	function PLAddTag($id, $tag, $value) {
		if ( $this->debugging ) addLog("mpd->PLAddTag()");
		if ( ! is_numeric($id) ) {
			$this->errStr = "PLAddTag() : argument 1 must be a numeric value";
			return NULL;
		}
		if ( ! is_null($resp = $this->SendCommand(MPD_CMD_ADD_TAG_ID,$id, $tag, $value))) $this->RefreshInfo();
		if ( $this->debugging ) addLog("mpd->PLAddTag() / return");
		return $resp;
	}
	...
	function SetRepeat($repVal) {
		if ( $this->debugging ) addLog("mpd->SetRepeat()");
		$rpt = $this->SendCommand(MPD_CMD_REPEAT,$repVal);
		$this->repeat = $repVal;
		if ( $this->debugging ) addLog("mpd->SetRepeat() / return");
		return $rpt;
	}
	...
	function SetRandom($rndVal) {
		if ( $this->debugging ) addLog("mpd->SetRandom()");
		$resp = $this->SendCommand(MPD_CMD_RANDOM,$rndVal);
		$this->random = $rndVal;
		if ( $this->debugging ) addLog("mpd->SetRandom() / return");
		return $resp;
	}
	 ...
	 */
	function Shutdown() {
		if ( $this->debugging ) addLog("mpd->Shutdown()");
		...
		if ( $this->debugging ) addLog("mpd->Shutdown() / return");
		return $resp;
	}
	 ...
	 */
	function DBRefresh() {
		if ( $this->debugging ) addLog("mpd->DBRefresh()");
		$resp = $this->SendCommand(MPD_CMD_REFRESH);
		
		// Update local variables
		$this->RefreshInfo();

		if ( $this->debugging ) addLog("mpd->DBRefresh() / return");
		return $resp;
	}
	 ...
	 */
	function Play() {
		if ( $this->debugging ) addLog("mpd->Play()");
		if ( ! is_null($rpt = $this->SendCommand(MPD_CMD_PLAY) )) $this->RefreshInfo();
		if ( $this->debugging ) addLog("mpd->Play() / return");
		return $rpt;
	}
	 ...
	 */
	function Stop() {
		if ( $this->debugging ) addLog("mpd->Stop()");
		if ( ! is_null($rpt = $this->SendCommand(MPD_CMD_STOP) )) $this->RefreshInfo();
		if ( $this->debugging ) addLog("mpd->Stop() / return");
		return $rpt;
	}
	 ...
	 */
	function Pause() {
		if ( $this->debugging ) addLog("mpd->Pause()");
		if ( ! is_null($rpt = $this->SendCommand(MPD_CMD_PAUSE) )) $this->RefreshInfo();
		if ( $this->debugging ) addLog("mpd->Pause() / return");
		return $rpt;
	}
	 ...
	 * 
	 * Skips directly to the <idx> song in the MPD playlist. 
	 * <idx> is a playlist id
	 */
	function SkipToId($idx) { 
		if ( $this->debugging ) addLog("mpd->SkipTo()");
		if ( ! is_numeric($idx) ) {
			$this->errStr = "SkipToId() : argument 1 must be a numeric value";
			return NULL;
		}
		if ( ! is_null($rpt = $this->SendCommand(MPD_CMD_PLAYID,$idx))) $this->RefreshInfo();
		if ( $this->debugging ) addLog("mpd->SkipTo() / return");
		return $idx;
	}
	...
	function SeekTo($pos, $track = -1) { 
		if ( $this->debugging ) addLog("mpd->SeekTo()");
		if ( ! is_numeric($pos) ) {
		...
		if ( $this->debugging ) addLog("mpd->SeekTo() / return");
		return $pos;
	}
	 ...
	 */
	function Next() {
		if ( $this->debugging ) addLog("mpd->Next()");
		if ( ! is_null($rpt = $this->SendCommand(MPD_CMD_NEXT))) $this->RefreshInfo();
		if ( $this->debugging ) addLog("mpd->Next() / return");
		return $rpt;
	}
	 ...
	 */
	function Previous() {
		if ( $this->debugging ) addLog("mpd->Previous()");
		if ( ! is_null($rpt = $this->SendCommand(MPD_CMD_PREV))) $this->RefreshInfo();
		if ( $this->debugging ) addLog("mpd->Previous() / return");
		return $rpt;
	}
	...
	function Find($type,$string) {
		if ( $this->debugging ) addLog("mpd->Find()");
		if ( $type != MPD_SEARCH_ARTIST and
	         $type != MPD_SEARCH_ALBUM and
			 $type != MPD_SEARCH_TITLE ) {
		...
		if ( $this->debugging ) addLog("mpd->Find() / return ".print_r($searchlist)."");
		return $searchlist;
	}
	 ...
	 */
	function Disconnect() {
		if ( $this->debugging ) addLog("mpd->Disconnect()");
		fclose($this->mpd_sock);
	...
	*/
	function GetArtists() {
		if ( $this->debugging ) addLog("mpd->GetArtists()");
	...
	*/
	function GetAlbums( $ar = NULL) {
		if ( $this->debugging ) addLog("mpd->GetAlbums()");
  	...
  	/* GetPlaylists() 
	 * 
	 * Retrieves all saved playlists from the server and tosses it into a multidimensional array.
     *
	 */
	function GetPlaylists() {
		if ( $this->debugging ) addLog("mpd->GetPlaylists()");
		$resp = $this->SendCommand(MPD_CMD_PLAYLISTS);
		$playlists = $this->_parseFileListResponse($resp);
		if ( $this->debugging ) addLog("mpd->GetPlaylists() / return ".print_r($playlists,true)."");
        // Only return the playlists. The other stuff is just noise
		return $playlists['playlists'];
	}

  	/* GetPlaylist($list) 
	 * 
	 * Retrieves the details of a single playlist
     *
	 */
	function GetPlaylist($list) {
		if ( $this->debugging ) addLog("mpd->GetPlaylist()");
		$resp = $this->SendCommand(MPD_CMD_PLAYLISTINFO, $list);
		$playlist = $this->_parseFileListResponse($resp);
		if ( $this->debugging ) addLog("mpd->GetPlaylist() / return ".print_r($playlist,true)."");
        // Only return the file info. The other stuff is just noise
	...
	*/
	...
	private function _parseFileListResponse($resp) {
		...
		addLog( "_parseFileListResponse() " . print_r($valuesArray,true) );
	...
	function GetStatistics() {
		if ( $this->debugging ) addLog("mpd->GetStatistics()");
		...
		if ( $this->debugging ) addLog("mpd->GetStatistics() / return: " . print_r($statsArray) ."");
		return $statsArray;
	}
	 ...
	 */
	function GetStatus() {
		if ( $this->debugging ) addLog("mpd->GetStatus()");
		...
		if ( $this->debugging ) addLog("mpd->GetStatus() / return: " . print_r($statusArray) ."");
		return $statusArray;
	}
	 ...
	 */
	function GetVolume() {
		if ( $this->debugging ) addLog("mpd->GetVolume()");
		...
		if ( $this->debugging ) addLog("mpd->GetVolume() / return: $vol");
		return $vol;
	}
	 ...
	 *   will most likely be deprecated in future releases.
	 */
	function oldGetPlaylist() {
		if ( $this->debugging ) addLog("mpd->GetPlaylist()");
		$resp = $this->SendCommand(MPD_CMD_PLLIST);
		$playlist = $this->_parseFileListResponse($resp);
		if ( $this->debugging ) addLog("mpd->GetPlaylist() / return ".print_r($playlist)."");
		return $playlist;
	}
	...
	var $COMPATIBILITY_MIN_TBL = array(
		...
		MPD_CMD_ADD_TAG_ID  => "0.19.0",
		MPD_CMD_DEL_TAG_ID  => "0.19.0"
